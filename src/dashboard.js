...
// Shows confidence scores, source contributions, and per-parameter agreement signals
...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,700&family=Manrope:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />
...
<header>
  <div class="brand">
    <h1>Krishi <span class="accent">Weather</span></h1>
    <p>4-source ensemble with ranking · for dryland decisions</p>
  </div>
  <div class="status">
...
<section id="ensemble-section" style="display: none;">
  <header>
    <h2>Parameter ensemble</h2>
    <span style="font-size:0.7rem;color:var(--text-faint);">all 4 sources ranked</span>
  </header>
...
<footer>
  <p style="margin-bottom:1rem;">
    ...
    <span style="display:inline-block;margin-top:0.5rem;">
      🟢 <strong>High (>0.85):</strong> sources strongly agree, trust the value. ·
      🟡 <strong>Moderate (0.6–0.85):</strong> some spread, treat as a range. ·
      🔴 <strong>Low (<0.6):</strong> sources disagree, examine individual contributions.
    </span>
  </p>
  <p>
...
(() => {
  ...
  async function reverseGeocode() {
    try {
      ...
      // Silent fail; doesn't block location selection
    }
  }
  ...
  function renderEnsemble() {
    ...
    for (const [param, result] of Object.entries(params)) {
      ...
      const unit = parameterUnit(param);
      ...
      let row = '<div class="param-row">' +
        '<div class="param-name">' + formatParamName(param) + '</div>' +
  ...
  function formatParamName(param) {
    const names = {
      ...
      rainfall_forecast: "Rainfall forecast (today)",
      climatology: "Normal rainfall (this month)",
      wind: "Wind speed",
      humidity: "Humidity",
      et0: "ET₀ (water demand)",
    };
  ...
  async function load() {
    ...
    setStatus("loading", "fetching ensemble…");
    try {
      ...
      setStatus("live", "live · cached at edge");
  ...
  $("gps").addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => showError("GPS failed: " + err.message),
